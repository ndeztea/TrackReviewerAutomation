	...
	<?php $a=0;$no=1;foreach($arrLogs['testsuite']['testsuite'] as $rowLogs1):?>
		...
		<tr class="subTest black">
			...
			<?php 
				...
				if (isset($rowLogs2['@attributes'])) $testSummarySuites2 = $rowLogs2['@attributes'];
				else $testSummarySuites2 = '';
			
				if (isset($testSummarySuites2['tests']) && $testSummarySuites2['tests'] > 0) { 
			...
			<tr class="subTestCase">
				...
				<td>- <?php if (isset($testSummarySuites2['name'])) echo $testSummarySuites2['name']?></td>
				<td><?php if (isset($testSummarySuites2['file'])) echo $testSummarySuites2['file']?></td>
				<td><?php if (isset($testSummarySuites2['tests'])) echo $testSummarySuites2['tests']?></td>
				<td><?php if (isset($testSummarySuites2['assertions'])) echo $testSummarySuites2['assertions']?></td>
				<td class="green"><?php echo $totalSuccess?> (<?php echo number_format($percentSuccess,2,'.',',')?>%)</td>
				<td class="yellow"><?php if (isset($testSummarySuites2['failures']))echo $testSummarySuites2['failures']?> (<?php echo number_format($percentFail,2,'.',',')?>%)</td>
				<td class="red"><?php if (isset($testSummarySuites2['errors'])) echo $testSummarySuites2['errors']?> (<?php echo number_format($percentError,2,'.',',')?>%)</td>
				<td><?php if(isset($testSummarySuites2['time'])) echo gmdate('H:i:s',$testSummarySuites2['time'])?></td>
			</tr>

			<?php if(!isset($testCase['testcase']['@attributes']) && isset($rowLogs2['testcase'])): ?>
				<?php $subNo2=1; foreach($rowLogs2['testcase'] as $testCases):?>
				<?php 
					
					if(isset($rowLogs2['testcase']['@attributes'])){
						$testCase = $rowLogs2['testcase'];
					}else{
						$testCase = $testCases;
					}
					$summaryTestCase = $testCase['@attributes'];	
					$class = 'green';
				...
				?>
					<tr class="<?php echo $class?>">
						...
						<td><a href="../screenshoot/<?php echo $date_log ?>/<?php echo $summaryTestCase['name']?>" target="_blank">-- <?php echo $summaryTestCase['name']?></a></td>
